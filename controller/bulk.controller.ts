...
export const getTenantsForBulk = async (req: Req, res: Res) => {
  try {
    const ownerId = (req as any).user.id as string;
    const ownerObjectId = new mongoose.Types.ObjectId(ownerId);
    const { propertyId } = req.query;

    const filter: any = { owner: ownerObjectId, status: "সক্রিয়" };
...
export const generateBulkInvoices = async (req: Req, res: Res) => {
  try {
    const ownerId = (req as any).user.id as string;
    const ownerObjectId = new mongoose.Types.ObjectId(ownerId);
    ...
    const tenants = await Tenant.find({
      _id: { $in: tenantIds.map((id: string) => new mongoose.Types.ObjectId(id)) },
      owner: ownerObjectId,
      status: "সক্রিয়",
    });
    ...
    for (const tenant of tenants) {
      try {
        ...
        await Invoice.create({
          ...
          status: "Unpaid",
          dueDate: new Date(Number(year), MONTH_NAMES.indexOf(month as string) + 1, 5),
        });
        results.created++;
      } catch (err: any) {
...
export const toggleAutoRenewal = async (req: Req, res: Res) => {
  try {
    const ownerId = (req as any).user.id as string;
    ...
    const tenant = await Tenant.findOneAndUpdate(
      { _id: tenantId, owner: ownerId },
      { autoRenew, renewalMonths: renewalMonths ?? 12 },
      { new: true }
    );
...
export const manualRenewLease = async (req: Req, res: Res) => {
  try {
    const ownerId = (req as any).user.id as string;
    const { tenantId } = req.params;
...
export const getExpiringLeases = async (req: Req, res: Res) => {
  try {
    const ownerId = (req as any).user.id as string;
    const ownerObjectId = new mongoose.Types.ObjectId(ownerId);
