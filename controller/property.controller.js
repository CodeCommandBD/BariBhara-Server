...
export const createProperty = async (req, res) => {
    try {
        ...
        const imageUrls = files && files.length > 0 ? files.map((file) => file.path.replace(/\\/g, "/")) : []; // স্ল্যাশ ফিক্স করা হলো
        ...
        const property = new Property({
            ...
            totalFloors,
            images: imageUrls,
            owner: req.user.id, // যদি ইউজার লগইন করা থাকে
        });
        // প্রপার্টি সেভ করা
        await property.save();
        // রেসপন্স পাঠানো
...
export const getMyProperties = async (req, res) => {
    try {
        const ownerId = req.user.id; // login করা ইউজারের আইডি
