...
export const verifyOTP = async (req, res) => {
    try {
        ...
        if (!stored)
            return res.status(400).json({ success: false, message: "OTP পাওয়া যায়নি বা মেয়াদ শেষ" });
...
export const verifyLoginOTP = async (req, res) => {
    try {
        ...
        if (!userId || !otp)
            return res.status(400).json({ success: false, message: "userId এবং OTP প্রয়োজন" });
        ...
        if (deviceId) {
            ...
            await User.findByIdAndUpdate(userId, {
                ...
                role: user.role === "landload" ? "landlord" : user.role
            });
...
export const sendLoginOTP = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ success: false, message: "2FA সক্রিয় নয়" });
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
