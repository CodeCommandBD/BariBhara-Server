...
export const generateInvoice = async (req, res) => {
    try {
        ...
        const { tenantId, month, year, waterBill, gasBill, electricityBill, serviceCharge, otherBill, dueDate } = req.body;
...
export const collectPayment = async (req, res) => {
    try {
        ...
        await sendNotification({
            ...
            message: `${invoice.tenant?.name} থেকে ${paymentAmount.toLocaleString()} টাকা গ্রহণ করা হয়েছে — ${invoice.month} ${invoice.year}`,
            link: "/payments",
        });
        // ২️⃣ ভাড়াটিয়াকে নোটিফিকেশন পাঠানো
        await sendNotification({
            recipient: String(invoice.tenant?._id),
            type: "payment",
            ...
            (async () => {
                try {
                    const pdfBuffer = await generateInvoicePDF({
                        ...
                        ownerName: owner?.fullName ?? "মালিক",
...
export const getPendingInvoices = async (req, res) => {
    try {
        ...
        const statusFilter = req.query.status; // "all" | "Unpaid" | "Partial" | "Paid"
        const monthFilter = req.query.month;
        const yearFilter = req.query.year;
        const baseFilter = { owner: new mongoose.Types.ObjectId(ownerId) };
        if (statusFilter && statusFilter !== "all") {
            baseFilter.status = statusFilter;
        }
        if (monthFilter)
            baseFilter.month = monthFilter;
        if (yearFilter)
            baseFilter.year = Number(yearFilter);
        ...
        // Build stats map
        const statsMap = { Unpaid: { count: 0, totalDue: 0 }, Partial: { count: 0, totalDue: 0 }, Paid: { count: 0, totalDue: 0 } };
        stats.forEach((s) => { if (statsMap[s._id])
            statsMap[s._id] = { count: s.count, totalDue: s.totalDue }; });
        res.status(200).json({
            success: true,
            invoices,
            total,
...
export const getInvoiceTransactions = async (req, res) => {
    try {
