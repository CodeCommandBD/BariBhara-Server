...
export const getFinancialReport = async (req, res) => {
    try {
        ...
        // ট্রানজেকশন ফিল্টার
        const txnFilter = { owner: ownerObjectId };
        if (startDate || endDate) {
            txnFilter.paymentDate = {};
            if (startDate)
                txnFilter.paymentDate.$gte = new Date(startDate);
            if (endDate)
                txnFilter.paymentDate.$lte = new Date(endDate);
        }
        // ইনভয়েস ফিল্টার
        const invFilter = { owner: ownerObjectId };
        if (propertyId && mongoose.Types.ObjectId.isValid(propertyId)) {
            invFilter.property = new mongoose.Types.ObjectId(propertyId);
        ...
        // মোট খরচ (Expense)
        const expFilter = { owner: ownerObjectId };
        if (startDate || endDate) {
            expFilter.expenseDate = {};
            if (startDate)
                expFilter.expenseDate.$gte = new Date(startDate);
            if (endDate)
                expFilter.expenseDate.$lte = new Date(endDate);
        }
        if (propertyId && mongoose.Types.ObjectId.isValid(propertyId)) {
            expFilter.property = new mongoose.Types.ObjectId(propertyId);
        }
...
export const getPropertiesForFilter = async (req, res) => {
    try {
        const ownerId = req.user.id;
...
export const exportTransactionsCSV = async (req, res) => {
    try {
        ...
        const filter = { owner: ownerObjectId };
        if (startDate || endDate) {
            filter.paymentDate = {};
            if (startDate)
