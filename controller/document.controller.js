...
export const uploadDocument = async (req, res) => {
    try {
        ...
        const file = req.file;
        if (!file)
            return res.status(400).json({ success: false, message: "ফাইল প্রয়োজন" });
        ...
        const result = await new Promise((resolve, reject) => {
            const stream = cloudinary.v2.uploader.upload_stream({ folder: `bari-bhara/documents/${ownerId}/${tenantId}`, resource_type: "auto" }, (error, result) => { if (error)
                reject(error);
            else
                resolve(result); });
...
export const getDocuments = async (req, res) => {
    try {
        ...
        if (!tenant)
            return res.status(404).json({ success: false, message: "ভাড়াটিয়া পাওয়া যায়নি" });
        res.status(200).json({ success: true, documents: tenant.documents ?? [] });
    }
    catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
...
export const deleteDocument = async (req, res) => {
    try {
        ...
        tenant.documents = (tenant.documents ?? []).filter((d) => d.publicId !== decodedPublicId);
        await tenant.save();
...
export const generateLeasePDF = async (req, res) => {
    try {
        ...
        doc.fontSize(11).font("Helvetica-Bold").text(`BDT ${(tenant.advanceAmount ?? 0).toLocaleString()}/-`, 220, gridY + 15);
        ...
        const end = tenant.leaseEnd ? new Date(tenant.leaseEnd).toLocaleDateString("en-BD") : "—";
        ...
        doc.text(`* Auto Renewal: ${tenant.autoRenew ? "Enabled" : "Disabled"}`, 70, gridY + 65);
        // 5. Signatures
        const sigY = 550;
