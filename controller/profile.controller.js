...
export const updateProfile = async (req, res) => {
    try {
        ...
        const updated = await User.findByIdAndUpdate(userId, { fullName, phone, bio, agreementTemplate }, { new: true, runValidators: true }).select("-password");
        res.status(200).json({
            success: true,
...
export const updateProfilePhoto = async (req, res) => {
    try {
        ...
        const updated = await User.findByIdAndUpdate(userId, { photo: result.secure_url }, { new: true }).select("-password");
        res.status(200).json({
            success: true,
