...
export const getLandlordStats = async (req, res) => {
    try {
        ...
        const [totalProperties, totalUnits, rentedUnits, monthlyRevenueData, totalDueData, pendingMaintenance,] = await Promise.all([
...
export const getLeaseExpiryAlerts = async (req, res) => {
    try {
        const ownerId = req.user.id;
